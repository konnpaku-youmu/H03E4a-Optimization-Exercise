function [x, x_iters, f_iters] = minimize_eq_sqp( ffun, hfun, x0, opts)
    ...
    for k=1:max_iters
        ...
        if max(abs(x)) > 1e6
            error('minimize_eq_sqp has diverged, max(abs(x)): %.3g',max(abs(x)))
        end
        ...
        fprintf('iteration: %4d,   norm_grad_L: %.2e \n',k, norm_grad_L);
...
%         lambda = sol(length(x)+1:end);
        ...
        qpopts = optimset('Display', 'off');
        qpopts.Algorithm='trust-region-reflective';
        
        [dk,~,~,~,l]  = quadprog(B, J', [], [], J_h, -h, [],[],[], qpopts);
        lambda = l.eqlin;
...
%         beta  = 0.6;
%         c     = 100;
%         x     = line_search(@(y)(merit_fun(ffun,hfun,c,y)), x, J*dk - c*norm(h,1), dk, sigma, beta);
...
%         y_tilde = y;
    ...
    error('minimize_eq_sqp: max iterations exceeded')
end
