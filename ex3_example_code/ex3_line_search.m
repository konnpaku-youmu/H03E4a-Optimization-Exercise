...
disp('minimizing using line search')
...
% now we solve
x0 = [1;1];
[xopt,x_iters,f_iters] = minimize_line_search(@(x)objective_toy(x(1), x(2)), x0);
...
% solve
x0 = [-1.5;1];
[xopt,x_iters, f_iters] = minimize_line_search(@(x)objective_rosenbrock(x(1), x(2)), x0);
